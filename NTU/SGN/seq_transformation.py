...
def split_dataset_dict(skes_joints, label, performer, camera, evaluation, save_path):
    train_indices, test_indices = get_indices(performer, camera, evaluation)
    m = 'sklearn'  # 'sklearn' or 'numpy'
    train_indices, val_indices = split_train_val(train_indices, m)

    # Create dictionaries
    train_data = {l: skes_joints[i] for i, l in zip(train_indices, label[train_indices])}
    val_data = {l: skes_joints[i] for i, l in zip(val_indices, label[val_indices])}
    test_data = {l: skes_joints[i] for i, l in zip(test_indices, label[test_indices])}

    # Save full dataset
    with open(os.path.join(save_path, 'X_full.pkl'), 'wb') as f:
        pickle.dump({l: skes_joints[i] for i, l in enumerate(label)}, f)

    # Save datasets
    for data, name in [(train_data, f'X_train_{evaluation}.pkl'), (val_data, f'X_val_{evaluation}.pkl'), (test_data, f'X_test_{evaluation}.pkl')]:
...
if __name__ == '__main__':
    ...
    #     split_dataset_dict(skes_joints, skes_name, performer, camera, evaluation, save_path)

    # Save to pickle where filename is key and value is skeleton
    X_full = {name: skes_joints[i] for i, name in enumerate(skes_name)}
    with open(os.path.join(save_path, 'X_full.pkl'), 'wb') as f:
